...
#reader(lib "htdp-advanced-reader.ss" "lang")((modname same-house-as-parent-v7-listing) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #t #t none #f () #f)))
...
; template from wizard arbitary arity tree wrapped in local
;           added work list accumulator for tail recursion
;           added result so far accumulator for tail recursion
;           added compound data defenition for wish list entries

(define (same-house-as-parent w)
  ;; todo is (listof ...) a worklist accumnulator
  ;; rsf is (listof String) a result so far accumulator
  (local [
          (define-struct wle (w ph))
          ;; WLE (work list entry) is (make-wle Wizard String)
          ;; interp. a worklist entry with the wiard
