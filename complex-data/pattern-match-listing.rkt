...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname pattern-match-listing) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
...
;; produce true if pattern match as described above
(check-expect (pattern-match? (list "A") empty) false)
(check-expect (pattern-match? (list "A" "N") (list "x" "4" "z")) true)
(check-expect (pattern-match? (list "A" "N") (list "x" "y" "z")) false)
(check-expect (pattern-match? (list "A") (list "x" "y" "z")) true)
(check-expect (pattern-match? (list "A") (list "5" "y" "z")) false)
(check-expect (pattern-match? (list "A" "N" "A" "N" "A" "N")
                (list "V" "6" "T" "1" "Z" "4")) true)
(check-expect (pattern-match? (list "A" "N" "N" "N" "A" "N")
                (list "V" "6" "T" "1" "Z" "4")) false)
;(define (pattern-match? p l1s) false) ; stub 
(define (pattern-match? p l1s)
  (cond
    [(empty? p) true]
    [(empty? l1s) false]
    [else
     (and
      (if (string=? (first p) "A")
          (alphabetic? (first l1s))
          (numeric? (first l1s)))
      (pattern-match? (rest p) (rest l1s)))]))
