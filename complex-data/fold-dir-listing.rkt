...
#reader(lib "htdp-intermediate-reader.ss" "lang")((modname fold-dir-listing) (read-case-sensitive #t) (teachpacks ()) (htdp-settings #(#t constructor repeating-decimal #f #t none #f () #f)))
(require 2htdp/image)
...
(define (count-images d)
  (local [(define (c1 img rloi)
            (+ 1 rloi))
          (define (c2 rdir rlod)
            (+ rdir rlod))
          (define (c3 n rlod rloi)
            (+ rlod rloi))]
...
(define (find name dir)
  (local[(define (c1 img rimgs)
           false)
         (define (c2 rdir rdirs)
           (or rdir rdirs))
         (define (c3 n rdirs rimgs)
           (or (string=? name n)
               rdirs
               rimgs))]
